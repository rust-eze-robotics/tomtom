use robotics_lib::interface::{Tools, go, teleport};
...
impl TomTom
{
    ...
    pub fn go_to_coordinate(&self, robot: &mut impl Runnable, world: &mut World, adjacent: bool, destination: (usize, usize)) -> Result<(), String> {
                    ...
                    match action {
                        ...
                        Action::Teleport((row, col)) => {
                            if let Err(_) = teleport(robot, world, (row, col)) {
                                return Err(String::from("Error while calling teleport interface!"));
                            }
                        }
                    }
                }
    ...
    pub fn go_to_tile(&self, robot: &mut impl Runnable, world: &mut World, adjacent: bool, tile_type: Option<TileType>, content: Option<Content>) -> Result<(), String> {
